...
const VS_VITE_ENTRY = 'main.js';

function vs_vite_server_url(): ?string
{
  $hot_path = get_theme_file_path('/.hot');
...
{
  $vite_manifest = get_theme_file_path('/dist/.vite/manifest.json');
...
add_action('wp_enqueue_scripts', static function (): void {
  ...
  if (!empty($entry['file'])) {
    wp_enqueue_script('vs-main', get_theme_file_uri('/dist/' . $entry['file']), [], null, true);
  }

  if (!empty($entry['css']) && is_array($entry['css'])) {
    foreach ($entry['css'] as $i => $css_file) {
      wp_enqueue_style('vs-style-' . $i, get_theme_file_uri('/dist/' . $css_file), [], null);
    }
  }
}, 20);
